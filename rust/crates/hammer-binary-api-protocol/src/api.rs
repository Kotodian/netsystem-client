...
//! layout. The CRC follows the original block and dependency folding; message
//! IDs, service declarations, flags, and options are not payload CRCs.
...
pub trait Api: serde::Serialize + for<'de> serde::Deserialize<'de> {
    ...
    /// Populates the fields shared by every client request.
    fn set_request_header(&mut self, _id: u16, _client_index: u32, _context: u32) {}

    /// Returns the request or reply context when the declaration has one.
